...
class CustomArguments:
    ...
    lora: bool = field(default=False, metadata={"help": "Whether to enable lora to train the model"})

    lora_rank: int = field(default=16, metadata={"help": "The r value for lora"})
...
class DefaultCollator:
    ...
    def __init__(self, model: MyEmbeddingModel) -> None:
        self.model = model
...
def main():
    ...
    model = MyEmbeddingModel.from_pretrained(
        ...
        attention_dropout=custom_args.dropout,
        enable_lora=custom_args.lora,
    )

    if custom_args.lora:
        model.model = initialize_peft(
            model.model,
            lora_r=custom_args.lora_rank,
            lora_alpha=2 * custom_args.lora_rank,

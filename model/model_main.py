...
from model.bidirectional import (
    MistralBiModel,
    LlamaBiModel,
    GemmaBiModel,
    Qwen2BiModel,
...
class MyEmbeddingModel(nn.Module):
    def __init__(
        ...
        self.decoder = decoder # add decoder module
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        model = model_class.from_pretrained(model_name_or_path, output_hidden_states=True, **kwargs)

        # decoder method
        print(f"num_layers: {model.config.num_hidden_layers}")
        decoder = HCFModel(dim=model.config.hidden_size, num_layers=model.config.num_hidden_layers)
        decoder = decoder.bfloat16()

        if os.path.isdir(model_name_or_path) and os.path.exists(
            f"{model_name_or_path}/config.json"
        ):
            with open(f"{model_name_or_path}/config.json", "r") as fIn:
        ...
        if checkpoint_name_or_path is not None:
            state_dict = torch.load(f"{checkpoint_name_or_path}/model.pt")
            if enable_lora:
                lora_path = f"{checkpoint_name_or_path}/lora"
        ...
        return cls(model=model, tokenizer=tokenizer, decoder=decoder, enable_lora=enable_lora, **config)
    ...
    def forward(self, sentence_feature: Dict[str, Tensor]):
        ...
        reps = self.model(**sentence_feature) # outputs
        sentence_feature["embed_mask"] = embed_mask
        
        # hcf module
        hidden_states = reps.hidden_states
        embedding = self.decoder(hidden_states)
    ...
    def save(self, output_path, merge_before_save=False, save_config=True):
        output_file = f"{output_path}/model.pt"
        state_dict = {}

        # save trainable params in decoder
        if self.decoder is not None:
            decoder_state = {}
            for name, param in self.decoder.named_parameters():
                decoder_state[name] = param.detach().cpu()
            state_dict["decoder"] = decoder_state

        # save trainable params in model
        if self.enable_lora:
            self.model.save_pretrained(f"{output_path}/lora")
        else:
            model_state = {}
            for name, param in self.model.named_parameters():
                model_state[name] = param.detach().cpu()
            state_dict["model"] = model_state

        torch.save(state_dict, output_file)
        print(f"Decoder states saved to {output_file}")
